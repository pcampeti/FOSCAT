...
class SphereDownGeo(nn.Module):
    ...
    PATCHED VERSION: Includes disk caching and progress reporting for faster
    ...
    - mode="maxpool": non-linear max over available children (fast, no matrix build)
    """
    ...
    def __init__(
        ...
        use_cache=True,  # NEW: enable disk caching
        verbose=True,    # NEW: progress reporting
    ):
        super().__init__()
